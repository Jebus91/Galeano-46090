...
float jckSz10(float, float, float); // jacket size
float waiSz10(float, float, float); // waist size
...
//declare variables 
    ...
    cout<<"10 years later"<<endl;
     if(years >= 40){
    cout<< jckSz10(height, weight, years)<<" inches is your jacket size after 10 years"<<endl;
     }
     if(years >= 30){
    cout<< waiSz10(height, weight, years)<<" inches is your waist  size after 10 years"<<endl;
     }
    return 0;
}
...
float jckSz10(float height, float weight, float years){
     float chestSz;
    if(years >= 40){
        chestSz = ((height * weight) /288.0f) + (0.125f * ((years + 10) / 10));
    }else{
     chestSz = height * weight /288.0f;
    } return chestSz;
}
...
float waiSz10(float height, float weight, float years){
    float waistSz;
    if(years >= 30){
        waistSz = (weight / 5.7f) + (0.1f * (years + 10)) ;
    }else{waistSz = weight / 5.7f;     
    }   
    return waistSz;
}